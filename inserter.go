...
func (w *Inserter) Write(ctx context.Context, model interface{}) error {
	if w.Map != nil {
		...
		queryInsert, values := BuildToInsertWithSchema(w.tableName, m2, -1, w.BuildParam, w.boolSupport, w.ToArray)
		_, err := w.db.ExecContext(ctx, queryInsert, values...)
		return err
	}
	queryInsert, values := BuildToInsertWithSchema(w.tableName, model, -1, w.BuildParam, w.boolSupport, w.ToArray)
	_, err := w.db.ExecContext(ctx, queryInsert, values...)
	return err
}
